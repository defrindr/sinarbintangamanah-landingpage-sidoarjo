...
class NumberHelper
{
    ...
    private static function terbilang_get_str($i, $str, $len)
    {
        ...
        switch ($pos) {
            case 1:
                if (!static::terbilang_get_valid($str, $i - 1, $i, 1, 1)) $buf = $numA[(int) $str[$i]];
                break;
            case 2:
            case 5:
            case 8:
            ...
            case 10:
            case 13:
                if (static::terbilang_get_valid($str, $i - 2, $i)) {
                    if (!static::terbilang_get_valid($str, $i - 1, $i, 1, 1))
    ...
    public static function toTerbilang($nominal)
    {
        ...
        $len = strlen($str);
        for ($i = 0; $i < $len; $i++) {
            $buf = trim($buf) . " " . static::terbilang_get_str($i, $str, $len);
        }
    ...
    public static function toReadableHarga($str, $withSpan = FALSE)
    {
        return ($withSpan ? "<span style='display:none'>" . ($str < 0 ? "-" : "+") . str_pad(abs($str), 10, "0", STR_PAD_LEFT) . "</span>" : "") . "Rp " . number_format($str, 2, ",", ".");
    }

    public static function toReadableNumber($str, $withSpan = FALSE)
    {
