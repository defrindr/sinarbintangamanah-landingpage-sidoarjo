...
/**
 * ManagementGalleryController implements the CRUD actions for Gallery model.
 **/
class ManagementGalleryController extends \app\components\productive\DefaultActiveController
{
    ...
    public function __construct($id, $module, $config = [])
    {
        ...
        // $this->validation = new \app\validations\GalleryValidation();
    }
        ...
        try {
            if ($model->load($_POST)) :
                if ($model->validate()) :
                    ...
                    $response = $this->uploadImage($instance, 'gallery');
                    if ($response->success == true) {
                        $model->image = $response->fileName;
                    } else {
                        $model->addError('image', $response->message);
                        goto end;
                    }
                    $model->save();
        ...
        try {
            if ($model->load($_POST)) :
                if ($model->validate()) :
                    ...
                    if ($response->success == true) {
                        $model->image = $response->fileName;
                        $this->deleteFile($model->getOldAttribute('image'));
                    } else {
                        $model->addError('image', $response->message);
                        goto end;
                    }
