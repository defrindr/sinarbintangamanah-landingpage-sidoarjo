...
namespace app\template\giiant\generators\crud;
 ...
 * @since 2.0
 */
class BaseGenerator extends \yii\gii\Generator
{
    public $modelClass;
    ...
    public function getNameAttribute()
    {
        foreach ($this->getColumnNames() as $name) {
            if (!strcasecmp($name, 'name') || !strcasecmp($name, 'title') || !strcasecmp($name, 'nama') || !strcasecmp($name, 'judul')) {
                return $name;
            }
        }
    ...
    public function generateActiveField($attribute)
    {
        ...
        if(strposa($column->name, ['ikon','icon','image','gambar','logo'])){
            return "\$form->field(\$model, '$attribute', \app\components\Constant::COLUMN())->widget(\kartik\\file\FileInput::className(), [
        ...
        if(strposa($column->name,['tanggal','date','_at'])){
