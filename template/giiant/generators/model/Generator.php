...
namespace app\template\giiant\generators\model;
...
class Generator extends \yii\gii\generators\model\Generator
{
     ...
     */
    public $createdByColumn = 'created_by,dibuat_oleh';

    /**
     * @var string the name of the column where the user who updated the entry is stored
     */
    public $updatedByColumn = 'updated_by,diubah_oleh';
     ...
     * @var string the name of the column where the user who updated the entry is stored
     */
    public $createdAtColumn = 'created_at,dibuat_pada';

    /**
     * @var string the name of the column where the user who updated the entry is stored
     */
    public $updatedAtColumn = 'updated_at,diubah_pada';
    ...
    public function generateRules($table)
    {
        ...
        foreach ($table->columns as $index => $column) {
            ...
            $column_id = ($column->name === "id");
            $removeCol = ($this->useBlameableBehavior && $isBlameableCol)
                || ($this->useTimestampBehavior && $isTimestampCol) || $column_id;
            if ($removeCol) {
            ...
            foreach ($relations[$tableName] as $relationName => $relation) {
                ...
                if ($referencedClassName === $langClassName) {
                    ...
                    foreach ($langTableColumns as $columnName) {
                        if (
                            !in_array($columnName, $langTableKeys) and strcasecmp(
                                $columnName,
    ...
    {
        foreach (explode(",", $this->createdByColumn) as $_createdBy) {
            $createdBy = $table->getColumn($_createdBy) !== null ? $_createdBy : false;
            if ($createdBy != false) {
                break;
            }
        }

        foreach (explode(",", $this->updatedByColumn) as $_updatedBy) {
            $updatedBy = $table->getColumn($_updatedBy) !== null ? $_updatedBy : false;
            if ($updatedBy != false) {
                break;
            }
        }
    ...
    {
        foreach (explode(",", $this->createdAtColumn) as $_createdAt) {
            $createdAt = $table->getColumn($_createdAt) !== null ? $_createdAt : false;
            if ($createdAt != false) {
                break;
            }
        }

        foreach (explode(",", $this->updatedAtColumn) as $_updatedAt) {
            $updatedAt = $table->getColumn($_updatedAt) !== null ? $_updatedAt : false;
            if ($updatedAt != false) {
                break;
            }
        }
